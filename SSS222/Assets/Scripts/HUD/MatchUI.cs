...
public class MatchUI : MonoBehaviour{
    ...
    IEnumerator Check(){
        ...
        if(gameObject.name=="CoinsIMG"&&!GameSession.instance.anyUpgradesOn&&GameRules.instance.shopOn){transform.position=new Vector2(transform.position.x+1,transform.position.y);}
        if(gameObject.name=="CoinsText"&&!GameSession.instance.anyUpgradesOn&&GameRules.instance.shopOn){rt.position=new Vector2(rt.position.x+(124/3),rt.position.y);}
