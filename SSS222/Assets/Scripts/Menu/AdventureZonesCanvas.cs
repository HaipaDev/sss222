...
public class AdventureZonesCanvas : MonoBehaviour{
    ...
    void Start(){
        zoneIconChildObject.GetComponent<Button>().onClick.AddListener(()=>GSceneManager.instance.LoadAdventureZone(1));
        zoneIconChildObject.GetComponent<RectTransform>().anchoredPosition=zonesList[0].pos;

        for(var i=1;i<zonesList.Capacity;i++){
            var go=Instantiate(zoneIconChildObject,listContent);
            go.name="Zone_"+(i+1);
            go.GetComponent<RectTransform>().anchoredPosition=zonesList[i].pos;
            go.GetComponent<Button>().onClick.AddListener(()=>GSceneManager.instance.LoadAdventureZone(i));
