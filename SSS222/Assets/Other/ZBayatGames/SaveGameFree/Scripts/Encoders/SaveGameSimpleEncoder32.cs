...
namespace BayatGames.SaveGameFree.Encoders
{
	...
	/// </summary>
	public class SaveGameSimpleEncoder32 : ISaveGameEncoder
	{
		...
		public string Encode ( string input, string password )
		{
			...
			#else
			return Convert.ToBase32String ( Encoding.UTF8.GetBytes ( input ) );
			#endif
		}
		...
		{
			#if !UNITY_WSA || !UNITY_WINRT
			var cipherTextBytesWithSaltAndIv = Base32.FromBase32String ( input );
			...
			#else
			return Encoding.UTF8.GetString ( Base32.FromBase32String ( input ) );
			#endif
		}
